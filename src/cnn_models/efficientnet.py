...
def create_efficientnet_model(num_classes: int):
    ...
    if dataset_name_upper == "INBREAST":
        # INbreast data pipeline is expected to provide 3-channel images
        ...
        # CMMD data pipeline provides 1-channel grayscale images
        print("[INFO EfficientNet] Dataset is CMMD. Expecting 1-channel input, will concatenate to 3 channels.")
        inp_gray = Input(shape=(img_height, img_width, 1), name="Input_Grayscale_CMMD_EfficientNet")
        concatenated_rgb = Concatenate(name="CMMD_EfficientNet_Grayscale_to_RGB")([inp_gray, inp_gray, inp_gray])
        tensor_fed_to_efficientnet_base = concatenated_rgb
        final_model_input_layer = inp_gray  # The overall model's input is still a single channel

    else:
        # Default behavior for other datasets: assume 1-channel input and concatenate
        print(f"[INFO EfficientNet] Dataset '{config.dataset}' not specifically handled. Assuming 1-channel input, will concatenate.")
        inp_gray_default = Input(shape=(img_height, img_width, 1), name="Input_Grayscale_Default_EfficientNet")
        concatenated_default_rgb = Concatenate(name="Default_EfficientNet_Grayscale_to_RGB")([inp_gray_default, inp_gray_default, inp_gray_default])
        tensor_fed_to_efficientnet_base = concatenated_default_rgb
    ...
    # Instantiate EfficientNetB0 base model with pre-trained ImageNet weights
    ...
    x = Dropout(0.4, name="EfficientNet_Dropout_1")(x)  # Increased dropout for regularization
    ...
    if num_classes == 2:
        ...
        if config.verbose_mode:
            print(f"[WARNING create_efficientnet_model] num_classes is {num_classes}. Defaulting output to 1 neuron with sigmoid.")
