...
def main() -> None:
    ...
    """
    KEY_LABEL = 3
    df = pd.read_csv("../data/mini-MIAS/mini-MIAS/data_description.csv", header=None, index_col=0)
    ...
    for img_pgm in os.listdir("../data/mini-MIAS/mini-MIAS/images_original"):
        if img_pgm.endswith(".pgm"):
            img = cv2.imread("../data/mini-MIAS/mini-MIAS/images_original/{}".format(img_pgm))
            img_name = img_pgm.split(".")[0]
            label = df.loc[img_name].loc[3]
            if label == 'N':
                new_path = "../data/mini-MIAS/mini-MIAS/images_processed/normal_cases/{}.png".format(img_name)
            elif label == 'B ':
                new_path = "../data/mini-MIAS/mini-MIAS/images_processed/benign_cases/{}.png".format(img_name)
            elif label == 'M ':
                new_path = "../data/mini-MIAS/mini-MIAS/images_processed/malignant_cases/{}.png".format(img_name)
